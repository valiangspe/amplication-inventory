/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { TransactionsWhereInput } from "./TransactionsWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { TransactionsOrderByInput } from "./TransactionsOrderByInput";

@ArgsType()
class TransactionsFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => TransactionsWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => TransactionsWhereInput, { nullable: true })
  @Type(() => TransactionsWhereInput)
  where?: TransactionsWhereInput;

  @ApiProperty({
    required: false,
    type: [TransactionsOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [TransactionsOrderByInput], { nullable: true })
  @Type(() => TransactionsOrderByInput)
  orderBy?: Array<TransactionsOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { TransactionsFindManyArgs as TransactionsFindManyArgs };
